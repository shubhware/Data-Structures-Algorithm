public class PalindromeLinkedList {
    ...
    public void deleteNthNodeFromEnd(int n) {
        ...
        if (sz == n) { // if n == sz means removeFirst operation.
            head = head.next;
            return;
        }
        int i = 1;
        int iToFind = sz - n; // Nth node idx = (size-n+1) so prev node idx = (sz - n)
        Node prev = head;
        while (i < iToFind) { // size =5, Nth node =3, sz-n = 2
            prev = prev.next;
            i++;
        }
        prev.next = prev.next.next; // prev.next = Nth node.next

    }

    // helper function to find midNode for reversing half of Linked List
    public Node findMid(Node head) { // slow-fast concept
        ...
        return slow; // return midNode
    }

    public boolean isPalindrome() {
        if (head == null || head.next == null) { // if empty or size is 1 then return true.
            return true;
        }
        // step-1 : Find midNode.
        ...
        Node prev = null;
        Node curr = midNode; // reverse from mid.
        Node next;
        while (curr != null) {
            next = curr.next;
            curr.next = prev;
        ...
        // step-3 : Check if (left.data == right.data).
        while (right != null) {
